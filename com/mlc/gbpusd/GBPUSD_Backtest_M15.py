        ...
        try:
            ...
            if len(self.positions) > 0:
                ...
                if current_position['direction'] != signal and signal != 0:
                    ...
                    points = price_diff * 10000  # GBPUSD小数点后4位为一个点
...
def get_mt5_data_by_range(symbol="GBPUSD", timeframe="TIMEFRAME_M15", start_date=None, end_date=None):
    """
    从MT5获取指定时间范围的历史数据
...
def main(start_date=None, end_date=None):
    ...
    try:
        ...
        model = None
        try:
            model = EvoAIModel("gbpusd_trained_model.pkl")
            # 检查模型是否成功加载
            ...
            if X is not None and y is not None and len(X) > 0 and len(y) > 0:
                ...
                model.save_model("gbpusd_trained_model.pkl")
            else:
