...
# MT5连接参数
mt5_timeframe = mt5.TIMEFRAME_H1  # H1时间框架

# 立即刷新输出
sys.stdout.reconfigure(encoding='utf-8')
print("开始执行USDJPY MT5回测策略...")
sys.stdout.flush()
...
# ======================== 5. 核心交易逻辑 ========================
def run_backtest_from_mt5(symbol, timeframe, count):
    """从MT5数据运行回测"""
    # 从MT5获取历史数据
    df = fetch_mt5_data(symbol, timeframe, count)
    
    if df is None:
        print("无法获取MT5数据，回测终止")
        return None
...
if __name__ == "__main__":
    try:
        # 从MT5获取USDJPY最近10000根H1 K线进行回测
        results = run_backtest_from_mt5("USDJPY", mt5.TIMEFRAME_H1, 10000)
