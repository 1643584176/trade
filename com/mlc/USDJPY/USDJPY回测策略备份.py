...
def calculate_sl_tp(current_row, direction, atr_value):
    ...
    # 按多空方向计算点位
    if direction == "long":
        sl = current_row['当前开盘价'] - sl_distance
        tp = current_row['当前开盘价'] + tp_distance
    else:
        sl = current_row['当前开盘价'] + sl_distance
        tp = current_row['当前开盘价'] - tp_distance
...
def run_backtest(data_file):
    ...
    for i in range(1, len(df)):
        ...
        if current_position is None and daily_trades < max_daily_trades:
            ...
            # USDJPY特定过滤条件
            # 1. 波动率过滤：避免在波动率极低或极高时交易
            volatility_filter = (current_row['Volatility_Signal'] > df['Volatility_Signal'].quantile(0.2)) and \
                               (current_row['Volatility_Signal'] < df['Volatility_Signal'].quantile(0.8))
            
            # 2. 趋势过滤：只在趋势明确时交易
            trend_filter = abs(current_row['当前增减']) > current_row['ATR'] * 0.5
            ...
            # 3. 强趋势信号
            elif (abs(current_row['当前增减']) > current_row['ATR'] * 1.5 and
            ...
            if signal_type and direction:
                ...
                order_id_counter += 1
                daily_trades += 1
                print(f"[{current_time}] 开仓: {direction}, 信号: {signal_type}, 入场价: {entry_price}, 止损: {sl}, 止盈: {tp}, 手数: {current_lot}")
