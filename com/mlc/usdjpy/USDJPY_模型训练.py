...
def train_usdjpy_model():
    """
    训练 USDJPY 模型
    """
    try:
        logger.info("开始训练 USDJPY 模型...")
        ...
        # 动态导入回测文件中的类
        backtest_file = os.path.join(current_dir, "USDJPY_Backtest_M15.py")
        if not os.path.exists(backtest_file):
            logger.error(f"找不到回测文件: {backtest_file}")
            return False
        
        # 导入必要的类
        from USDJPY_Backtest_M15 import FeatureEngineer, EvoAIModel
        
        # 获取数据
        logger.info("获取历史数据...")
        try:
            df = get_mt5_data("USDJPY", "TIMEFRAME_M15", 365)
        ...
        # 保存模型
        model_file = os.path.join(current_dir, "usdjpy_trained_model.pkl")
        logger.info(f"保存模型到 {model_file}...")
        model.save_model(model_file)
        
        logger.info("USDJPY 模型训练完成")
        return True
        
    except Exception as e:
        logger.error(f"训练 USDJPY 模型时出错: {str(e)}")
        return False
...
def main():
    ...
    主函数
    """
    logger.info("=== USDJPY 模型训练程序启动 ===")
    
    success = train_usdjpy_model()
    
    if success:
        logger.info("=== USDJPY 模型训练成功完成 ===")
    else:
        logger.error("=== USDJPY 模型训练失败 ===")
