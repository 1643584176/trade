...
class RealTimeTraderM15:
    ...
    def __init__(self, model_path="trained_model.pkl"):
        ...
        self.magic_number = 10000002  # 魔法数字，用于区分不同品种的订单
    ...
    def _check_existing_positions(self, symbol):
        ...
        try:
            ...
            if len(filtered_positions) > 0:
                ...
                logger.info(
                    f"检测到现有持仓: {direction_str}, 入场价格: {self.current_position['entry_price']:.5f}, 手数: {self.current_position['lots']}")
            else:
    ...
    def close_position_mt5(self, symbol):
        ...
        try:
            ...
            # 创建平仓请求
            position_type = self.mt5.ORDER_TYPE_BUY if self.current_position[
                                                           'direction'] > 0 else self.mt5.ORDER_TYPE_SELL
            close_request = {
                "action": self.mt5.TRADE_ACTION_DEAL,
                "symbol": symbol,
    ...
    def execute_trade(self, symbol, signal, lot_size=2.0, current_price=None):
        """
        执行交易
        ...
        try:
            ...
            while self.is_running:
                try:
                    ...
                    if self.current_position is not None:
                        logger.info(
                            f"当前持仓方向: {'做多' if self.current_position['direction'] > 0 else '做空'}, 入场价格: {self.current_position['entry_price']:.5f}")
                    else:
                        logger.info("当前无持仓")
