...
def add_technical_indicators(df):
    ...
    # 计算RSI指标
    df['RSI'] = calculate_rsi(df['当前收盘价'], 14)
...
def calculate_sl_tp(current_row, direction, volatility_multiplier=1.0):
    """
    计算止损止盈（考虑点差的真实价格）
    ...
    Args:
        ...
        volatility_multiplier (float): 波动率调整系数
    ...
    # 根据波动率调整止损止盈
    sl_multiplier *= volatility_multiplier
    tp_multiplier *= volatility_multiplier

    # 按多空方向计算点位（考虑点差）
    spread_price = SPREAD_PIPS * POINT_SIZE
    if direction == "long":
        entry_price = current_row['当前开盘价'] + spread_price  # 买入价（考虑点差）
        sl = entry_price - sl_multiplier * current_row['当前波动幅度']
        tp = entry_price + tp_multiplier * current_row['当前波动幅度']
    else:
...
class AUDUSDStrategy:
    ...
    def generate_signal(self, df, i):
        ...
        if (pd.isna(latest_data['MA5']) or pd.isna(latest_data['MA10']) or 
            pd.isna(latest_data['MA20']) or pd.isna(latest_data['MA50']) or
            pd.isna(prev_data['MA5']) or pd.isna(prev_data['MA10']) or
            pd.isna(latest_data['RSI'])):
            return None, None
        ...
        print(f"[AUDUSD优化回测] 分析信号: 昨天{prev_weekday_chinese}, 今天{current_weekday_chinese}")
        
        # 趋势过滤：只有在趋势明确时才交易
        long_trend = latest_data['MA50'] < latest_data['MA20'] < latest_data['MA5']
        short_trend = latest_data['MA50'] > latest_data['MA20'] > latest_data['MA5']
        
        # RSI过滤：避免在超买超卖区域反向操作
        rsi_ok_long = latest_data['RSI'] < 70  # 做多时RSI不能过高
        rsi_ok_short = latest_data['RSI'] > 30  # 做空时RSI不能过低
        
        # 1. 周一→周二趋势延续（增加趋势和RSI过滤）
        if prev_weekday == 0 and current_weekday == 1:  # 周一→周二
            # 周一上涨→周二延续：做多
            if (prev_data['昨天增减'] > 0 and
                    latest_data['当前开盘价'] > prev_data['当前收盘价'] and
                    latest_data['当前增减'] > 0 and
                    long_trend and rsi_ok_long):
                print(f"[AUDUSD优化回测] 满足做多条件: 周一上涨周二延续")
                return "long", "周一上涨周二延续"

            # 周一下跌→周二延续：做空
            elif (prev_data['昨天增减'] < 0 and
                  latest_data['当前开盘价'] < prev_data['当前收盘价'] and
                  latest_data['当前增减'] < 0 and
                  short_trend and rsi_ok_short):
                print(f"[AUDUSD优化回测] 满足做空条件: 周一下跌周二延续")
                return "short", "周一下跌周二延续"
        ...
        # 2. 周三反转信号（增加趋势和RSI过滤）
            ...
            if (df.iloc[i-2]['当前增减'] < 0 and
                    ...
                    latest_data['当前增减'] > 0 and
                    long_trend and rsi_ok_long):
                print(f"[AUDUSD优化回测] 满足做多条件: 周三反转做多")
                return "long", "周三反转做多"
            ...
            # 前两日上涨→周三反转：做空
                  ...
                  latest_data['当前增减'] < 0 and
                  short_trend and rsi_ok_short):
                print(f"[AUDUSD优化回测] 满足做空条件: 周三反转做空")
                return "short", "周三反转做空"

        # 3. MA金叉/死叉（技术信号，增加趋势和RSI过滤）
        elif i >= 1:
            prev_ma_data = df.iloc[i-1]
            # MA金叉（增加趋势确认）
            if (latest_data['MA5'] > latest_data['MA10'] and 
                prev_ma_data['MA5'] <= prev_ma_data['MA10'] and
                long_trend and rsi_ok_long):
                print(f"[AUDUSD优化回测] 满足做多条件: MA金叉")
                return "long", "MA金叉"
            ...
            # MA死叉（增加趋势确认）
                  ...
                  prev_ma_data['MA5'] >= prev_ma_data['MA10'] and
                  short_trend and rsi_ok_short):
                print(f"[AUDUSD优化回测] 满足做空条件: MA死叉")
                return "short", "MA死叉"
    ...
    def execute_trade(self, signal_type, signal_direction, data_row, lot_size):
        ...
        if self.last_trade_result == "止损":
            # 如果上一笔交易止损，降低波动率系数以减小风险
            volatility_multiplier = 0.8
        
        # 计算止损止盈（考虑点差）
        sl, tp = calculate_sl_tp(data_row, signal_direction, volatility_multiplier)
        ...
        print(f"[AUDUSD优化回测] 开仓: {signal_direction}, 信号: {signal_type}, 入场价: {entry_price}, 止损: {sl}, 止盈: {tp}, 手数: {lot_size}")
    ...
    def check_exit_conditions(self, current_data, next_data):
        ...
        for position in self.positions[:]:  # 使用切片复制避免在迭代时修改列表
            ...
            if should_close:
                ...
                print(f"[AUDUSD优化回测] 平仓: {close_reason}, 盈亏: {pnl:.2f}, 余额: {self.balance:.2f}")
