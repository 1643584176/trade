...
for idx, current in df.iterrows():
    ...
    if current_position is None and current_lot > 0:
        ...
        if signal_type and direction:
            ...
            order_id_counter += 1
            print(f"[{current_time_str}] 开{'多' if direction=='long' else '空'}单{current_position.order_id}：{signal_type}，价{current['当前开盘价']}，"
