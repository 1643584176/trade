...
# ======================== 1. 策略参数 ========================
# 交易参数
INITIAL_BALANCE = 100000.0  # 初始资金改为10万
...
def load_historical_data(file_path):
    ...
    try:
        ...
        df = df.sort_values('时间点').reset_index(drop=True)
...
class AUDUSDStrategy:
    ...
    def generate_signal(self, df, i):
        ...
        print(f"[AUDUSD回测] 分析信号: 昨天{prev_weekday_chinese}, 今天{current_weekday_chinese}")
        ...
        if prev_weekday == 0 and current_weekday == 1:  # 周一→周二
            ...
            if (prev_data['昨天增减'] > 0 and
                    ...
                    latest_data['当前增减'] > 0):
                print(f"[AUDUSD回测] 满足做多条件: 周一上涨周二延续")
            ...
            # 周一下跌→周二延续：做空
                  ...
                  latest_data['当前开盘价'] < prev_data['当前收盘价'] and
                  latest_data['当前增减'] < 0):
                print(f"[AUDUSD回测] 满足做空条件: 周一下跌周二延续")
        ...
        # 2. 周三反转信号
            ...
            if (df.iloc[i-2]['当前增减'] < 0 and
                    ...
                    latest_data['当前开盘价'] > prev_data['当前收盘价'] and
                    latest_data['当前增减'] > 0):
                print(f"[AUDUSD回测] 满足做多条件: 周三反转做多")
                return "long", "周三反转做多"
            ...
            # 前两日上涨→周三反转：做空
                  ...
                  latest_data['当前开盘价'] < prev_data['当前收盘价'] and
                  latest_data['当前增减'] < 0):
                print(f"[AUDUSD回测] 满足做空条件: 周三反转做空")
                return "short", "周三反转做空"
        ...
        # 3. MA金叉/死叉（技术信号）
            ...
            if (latest_data['MA5'] > latest_data['MA10'] and 
                ...
                print(f"[AUDUSD回测] 满足做多条件: MA金叉")
                return "long", "MA金叉"
            
            # MA死叉
            elif (latest_data['MA5'] < latest_data['MA10'] and 
                  prev_ma_data['MA5'] >= prev_ma_data['MA10']):
                print(f"[AUDUSD回测] 满足做空条件: MA死叉")
                return "short", "MA死叉"
    ...
    def execute_trade(self, signal_type, signal_direction, data_row, lot_size):
        ...
        """
        # 计算止损止盈（考虑点差）
        sl, tp = calculate_sl_tp(data_row, signal_direction)
        ...
        print(f"[AUDUSD回测] 开仓: {signal_direction}, 信号: {signal_type}, 入场价: {entry_price}, 止损: {sl}, 止盈: {tp}, 手数: {lot_size}")
    ...
    def check_exit_conditions(self, current_data, next_data):
        ...
        for position in self.positions[:]:  # 使用切片复制避免在迭代时修改列表
            ...
            if should_close:
                ...
                print(f"[AUDUSD回测] 平仓: {close_reason}, 盈亏: {pnl:.2f}, 余额: {self.balance:.2f}")
...
def main():
    # 尝试直接从MT5获取数据
    print("正在尝试从MT5获取实时历史数据...")
    df = load_historical_data_from_mt5("AUDUSD", mt5.TIMEFRAME_H1, 730)
    
    # 如果无法从MT5获取数据，则尝试从CSV文件加载
    if df is None or len(df) == 0:
        print("无法从MT5获取数据，尝试从CSV文件加载...")
        csv_file = os.path.join(os.path.dirname(__file__), 'AUDUSD_H1_历史数据.csv')
        
        # 检查文件是否存在
        if not os.path.exists(csv_file):
            print(f"错误: 找不到历史数据文件 {csv_file}")
            print("请确保AUDUSD_H1_历史数据.csv文件存在于同一目录下")
            return
