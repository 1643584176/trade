...
class AUDUSDStrategy:
    ...
    def generate_signal(self, df, i):
        ...
        print(f"[AUDUSD优化v2回测] 分析信号: 昨天{prev_weekday_chinese}, 今天{current_weekday_chinese}")
        
        # 根据上一笔交易结果调整开仓策略
        if self.last_trade_result == "止盈":
            # 上一笔止盈，下一笔反向开仓
            print("[AUDUSD优化v2回测] 上一笔交易止盈，尝试反向开仓")
        elif self.last_trade_result == "止损":
            # 上一笔止损，下一笔顺势开仓
            print("[AUDUSD优化v2回测] 上一笔交易止损，尝试顺势开仓")
        ...
        if prev_weekday == 0 and current_weekday == 1:  # 周一→周二
            ...
            if (prev_data['昨天增减'] > 0 and
                ...
                if self.last_trade_result == "止盈":
                    direction = "short"  # 反向
                    signal_type += "(反向)"
                print(f"[AUDUSD优化v2回测] 满足做{direction}条件: {signal_type}")
    ...
    def execute_trade(self, signal_type, signal_direction, data_row, lot_size):
        ...
        print(f"[AUDUSD优化v2回测] 开仓: {signal_direction}, 信号: {signal_type}, 入场价: {entry_price}, 止损: {sl}, 止盈: {tp}, 手数: {lot_size}")
    ...
    def check_exit_conditions(self, current_data, next_data):
        ...
        for position in self.positions[:]:  # 使用切片复制避免在迭代时修改列表
            ...
            if should_close:
                ...
                print(f"[AUDUSD优化v2回测] 平仓: {close_reason}, 盈亏: {pnl:.2f}, 余额: {self.balance:.2f}")
