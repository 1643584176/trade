...
# 在文件顶部添加日期配置变量
START_DATE = '2025-10-01'   # 格式: '2024-06-01' 或 None 使用默认值
...
class Backtester:
        ...
        try:
            ...
            if len(self.positions) > 0:
                ...
                if current_position['direction'] != signal and signal != 0:
                    # 平仓 (BTCUSD每点价值1美元)
                    profit = (data['close'] - current_position['entry_price']) * current_position['direction'] * 1 * current_position['lots']
                    self.balance += profit
        ...
        try:
            ...
            if len(self.positions) > 0:
                for position in self.positions:
                    # 平仓 (BTCUSD每点价值1美元)
                    profit = (data['close'] - position['entry_price']) * position['direction'] * position['lots'] * 1
                    self.balance += profit
    ...
    def _update_equity(self, current_price, timestamp):
        ...
        try:
            equity = self.balance
            # 计算未平仓盈亏 (BTCUSD每点价值1美元)
            for position in self.positions:
                unrealized_pnl = (current_price - position['entry_price']) * position['direction'] * position['lots'] * 1
...
def get_mt5_data(symbol="BTCUSD", timeframe="TIMEFRAME_M15", days=365):
    """
    从MT5获取历史数据
...
def get_mt5_data_by_range(symbol="BTCUSD", timeframe="TIMEFRAME_M15", start_date=None, end_date=None):
    """
    从MT5获取指定时间范围的历史数据
...
def main(start_date=None, end_date=None):
    ...
    try:
        ...
        if parsed_start_date or parsed_end_date:
            df = get_mt5_data_by_range("BTCUSD", "TIMEFRAME_M15", parsed_start_date, parsed_end_date)
        else:
            df = get_mt5_data("BTCUSD")
        ...
        model = None
        try:
            model = EvoAIModel("btcusd_trained_model.pkl")
            # 检查模型是否成功加载
            ...
            if X is not None and y is not None and len(X) > 0 and len(y) > 0:
                ...
                model.save_model("btcusd_trained_model.pkl")
            else:
