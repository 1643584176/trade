...
def calculate_sl_tp(current_row, direction):
    ...
    print(f"[GBPUSD] 计算止盈止损: 方向={direction}, 入场价={current_row['开盘价']}, 止损={sl}, 止盈={tp}, 波动幅度={current_row['波动幅度']}")
    
    return sl, tp

def get_mt5_data(symbol="GBPUSD", timeframe=mt5.TIMEFRAME_H1, count=1000):
    """
    从MT5获取GBPUSD历史数据
    """
    try:
...
def initialize_trading_day():
    ...
    try:
        ...
        if history_orders is not None and len(history_orders) > 0:
            ...
            for order in history_orders:
                if hasattr(order, 'symbol') and order.symbol == symbol:
                    gbpusd_orders.append(order)
            
            # 计算今日GBPUSD已交易次数和盈亏
            # 每笔交易包含开仓和平仓两个订单，所以除以2
            daily_trades = len(gbpusd_orders) // 2
        ...
        print(f"[GBPUSD] 今日交易次数初始化: {daily_trades}, 今日盈亏: {daily_pnl:.2f}")
        
    except Exception as e:
        print(f"[GBPUSD] 初始化交易日时发生错误: {str(e)}")
...
def check_position_status():
    ...
    try:
                ...
                for order in history_orders:
                    if hasattr(order, 'symbol') and order.symbol == symbol:
                        ...
                        if hasattr(order, 'profit') and order.profit is not None:
                            ...
                            print(f"[GBPUSD] 检测到外部平仓，盈亏: {order.profit:.2f}, 余额: {current_balance:.2f}, 今日交易次数: {daily_trades}")
                            break
        ...
        # 如果有持仓但程序认为没有持仓，说明是外部开仓
            ...
            print(f"[GBPUSD] 检测到外部开仓，订单号: {position.ticket}, 方向: {'多' if position.type == 0 else '空'}, 手数: {position.volume}")
            
    except Exception as e:
        print(f"[GBPUSD] 检查持仓状态时发生错误: {str(e)}")
...
def print_time_info():
    ...
    try:
        ...
        if m1_rates is not None and len(m1_rates) > 0:
            ...
            print(f"[GBPUSD] MT5 M1时间: {TimeUtils.datetime_to_string(m1_time, True)}, 北京时间: {TimeUtils.datetime_to_string(beijing_time, True)}")
        ...
        if h1_rates is not None and len(h1_rates) > 0:
            ...
            print(f"[GBPUSD] MT5 H1时间: {TimeUtils.datetime_to_string(h1_time, True)}, 北京时间: {TimeUtils.datetime_to_string(beijing_time, True)}")
            
    except Exception as e:
        print(f"[GBPUSD] 获取时间信息时发生错误: {str(e)}")
...
def send_order(direction, lot_size, sl, tp, price):
    ...
    try:
        ...
        if price is None or sl is None or tp is None:
            print("[GBPUSD] 订单价格参数无效")
            return None
            
        # 验证手数
        if lot_size <= 0:
            print("[GBPUSD] 订单手数必须大于0")
            return None
            
        # 获取交易品种信息
        symbol_info = mt5.symbol_info(symbol)
        if symbol_info is None:
            print(f"[GBPUSD] 无法获取交易品种 {symbol} 的信息")
            return None
            
        # 检查交易品种是否可用
        if not symbol_info.visible:
            if not mt5.symbol_select(symbol, True):
                print(f"[GBPUSD] 无法选择交易品种 {symbol}")
                return None
                
        # 检查价格是否合理
        tick = mt5.symbol_info_tick(symbol)
        if tick is None:
            print("[GBPUSD] 无法获取当前报价")
            return None
        ...
        if direction == "long":
            ...
            if sl >= price or tp <= price:
                print(f"[GBPUSD] 多单止损或止盈价格设置不合理: 入场价={price}, 止损={sl}, 止盈={tp}")
                return None
        else:
            order_type = mt5.ORDER_TYPE_SELL
            # 检查止损和止盈是否合理 (空单止损应高于入场价，止盈应低于入场价)
            if sl <= price or tp >= price:
                print(f"[GBPUSD] 空单止损或止盈价格设置不合理: 入场价={price}, 止损={sl}, 止盈={tp}")
                return None
        ...
        # 打印下单信息
        print(f"[GBPUSD] 准备下单: 品种={symbol}, 方向={direction}, 手数={lot_size}, 入场价={price}, 止损={sl}, 止盈={tp}")
        
        # 发送订单
        result = mt5.order_send(request)
        if result.retcode != mt5.TRADE_RETCODE_DONE:
            print(f"[GBPUSD] 订单发送失败，错误码: {result.retcode}")
            # 打印更多错误信息帮助诊断
            print(f"[GBPUSD] 错误详情: {mt5.last_error()}")
            return None
            
        print(f"[GBPUSD] 订单发送成功，订单号: {result.order}")
        return result.order
        
    except Exception as e:
        print(f"[GBPUSD] 发送订单时发生错误: {str(e)}")
        return None
...
def close_position(ticket):
    ...
    try:
        ...
        result = mt5.order_send(request)
        if result.retcode != mt5.TRADE_RETCODE_DONE:
            print(f"[GBPUSD] 平仓失败，错误码: {result.retcode}")
            return False
            
        print(f"[GBPUSD] 平仓成功，订单号: {result.order}")
        return True
        
    except Exception as e:
        print(f"[GBPUSD] 平仓时发生错误: {str(e)}")
        return False
...
def run_strategy():
    ...
    while True:  # 持续运行循环
        try:
            ...
            # 从MT5获取最新数据
            print("正在从MT5获取GBPUSD最新历史数据...")
            df = get_mt5_data("GBPUSD", mt5.TIMEFRAME_H1, 2000)
            ...
            if m1_rates is not None and len(m1_rates) > 0:
                ...
                print(f"[GBPUSD] M1时间: {TimeUtils.datetime_to_string(m1_time, True)}, 北京时间: {TimeUtils.datetime_to_string(beijing_time, True)}, 星期: {weekday_chinese}({weekday_num})")
            else:
                print("[GBPUSD] 无法获取M1时间信息")
            ...
            if not shared_state.can_open_position():
                ...
                print(f"[GBPUSD] 风险控制限制开仓: 当前手数={current_lot}, 全局当日盈亏={global_stats['daily_pnl']:.2f}, 全局总盈亏={global_stats['total_pnl']:.2f}, 全局净值={global_stats['equity']:.2f}")
            else:
            ...
            if current_position is None and current_lot > 0:
                ...
                if last_close_time is not None:
                    ...
                    if last_close_time >= latest_h1_time:
                        print(f"[GBPUSD] 上次平仓时间 {last_close_time} 在当前K线周期内，等待下一根完整K线")
                        time.sleep(60)
                        continue
                ...
                print(f"[GBPUSD] 分析H1数据信号: 昨天星期={yesterday_weekday_chinese}, 今天星期={current_weekday_chinese}")
                ...
                if prev_data['星期几'] == 'Monday' and latest_data['星期几'] == 'Tuesday':
                    ...
                    if (prev_data['昨天增减'] > 0 and
                        ...
                        direction = "long"
                        print(f"[GBPUSD] 满足做多条件: 周一上涨周二延续 - 昨天增减={prev_data['昨天增减']:.5f}, 价格跳空={latest_data['开盘价'] > prev_data['收盘价']}, 当前上涨={latest_data['增减'] > 0}")
                    ...
                    # 周一下跌→周二延续：做空
                        ...
                        direction = "short"
                        print(f"[GBPUSD] 满足做空条件: 周一下跌周二延续 - 昨天增减={prev_data['昨天增减']:.5f}, 价格跳空={latest_data['开盘价'] < prev_data['收盘价']}, 当前下跌={latest_data['增减'] < 0}")
                    else:
                ...
                # 2. 周三反转信号
                    ...
                    if (df.iloc[-3]['增减'] < 0 and
                        ...
                        signal_type = "周三反转做多"
                        direction = "long"
                        print(f"[GBPUSD] 满足做多条件: 周三反转做多 - 前两日下跌({df.iloc[-3]['增减']:.5f}, {prev_data['增减']:.5f}), 价格跳空={latest_data['开盘价'] > prev_data['收盘价']}, 当前上涨={latest_data['增减'] > 0}")
                    ...
                    # 前两日上涨→周三反转：做空
                        ...
                        signal_type = "周三反转做空"
                        direction = "short"
                        print(f"[GBPUSD] 满足做空条件: 周三反转做空 - 前两日上涨({df.iloc[-3]['增减']:.5f}, {prev_data['增减']:.5f}), 价格跳空={latest_data['开盘价'] < prev_data['收盘价']}, 当前下跌={latest_data['增减'] < 0}")
                    else:
                ...
                # 3. MA金叉/死叉（技术信号）
                    ...
                    signal_type = "MA金叉"
                    direction = "long"
                    print(f"[GBPUSD] 满足做多条件: MA金叉 - 当前MA5={latest_data['MA5']:.5f}, MA10={latest_data['MA10']:.5f}, 前期MA5={prev_data['MA5']:.5f}, 前期MA10={prev_data['MA10']:.5f}")
                    
                elif latest_data['MA5'] < latest_data['MA10'] and prev_data['MA5'] >= prev_data['MA10']:
                    signal_type = "MA死叉"
                    direction = "short"
                    print(f"[GBPUSD] 满足做空条件: MA死叉 - 当前MA5={latest_data['MA5']:.5f}, MA10={latest_data['MA10']:.5f}, 前期MA5={prev_data['MA5']:.5f}, 前期MA10={prev_data['MA10']:.5f}")
                else:
                ...
                if signal_type and direction:
                    ...
                    if ticket is not None:
                        ...
                        order_id_counter += 1
                        # 简化输出信息
                        print(f"[GBPUSD] 开仓: {direction}, 满足条件: {signal_type}, 入场价: {price}, 止损: {sl}, 止盈: {tp}")
                else:
                    # 没有满足条件的信号，输出详细原因
                    if signal_reasons:
                        print("[GBPUSD] 未满足入场条件:")
                        for reason in signal_reasons:
                            print(f"  - {reason}")
                    else:
                        print("[GBPUSD] 未满足入场条件，当前日期不符合任何交易信号触发条件")
                    print("[GBPUSD] 继续等待")
            elif current_position is not None:
                print(f"[GBPUSD] 当前持有仓位: {current_position.direction}, 入场价: {current_position.entry_price}")
            elif current_lot <= 0:
                # 获取全局状态信息用于输出
                global_stats = shared_state.get_global_stats()
                print(f"[GBPUSD] 风险控制限制开仓: 当前手数={current_lot}, 全局当日盈亏={global_stats['daily_pnl']:.2f}, 全局总盈亏={global_stats['total_pnl']:.2f}, 全局净值={global_stats['equity']:.2f}")
            ...
            if current_position is not None:
                ...
                if positions is None or len(positions) == 0:
                    ...
                    print(f"[GBPUSD] 持仓已平仓，订单号: {current_position.ticket}")
                        ...
                        for order in history_orders:
                            if hasattr(order, 'symbol') and order.symbol == symbol:
                                ...
                                if hasattr(order, 'profit') and order.profit is not None:
                                    ...
                                    print(f"[GBPUSD] 平仓: MT5自动平仓, 盈亏: {profit:.2f}, 今日交易次数: {daily_trades}")
                                    # 记录平仓时间
                ...
                if current_position is not None:
                    ...
                    if time_diff.total_seconds() > 24 * 3600:  # 超过24小时
                        ...
                        if close_position(current_position.ticket):
                            ...
                            # 简化输出信息
                            print(f"[GBPUSD] 平仓: 时间止损, 盈亏: {position_copy.pnl:.2f}, 今日交易次数: {daily_trades}")
                            current_position = None
                            daily_trades += 1
