...
class M5ModelTrainer(BaseModelTrainer):
    ...
    def get_m5_historical_data(self, bars_count: int = 6000):
        """获取MT5真实历史M5数据"""
        self.initialize_mt5()
        ...
        # 创建目标变量：预测未来1根K线的涨跌 (1=涨, 0=跌, -1=平)
        df['future_close'] = df['close'].shift(-1)
        df['price_change_pct'] = (df['future_close'] - df['close']) / df['close']
        
        # 定义目标变量
        df['target'] = np.where(df['price_change_pct'] > 0.001, 1,  # 涨
                               np.where(df['price_change_pct'] < -0.001, -1, 0))  # 跌和平
    ...
    def train_model(self):
        """训练M5模型"""
        print("开始获取M5历史数据...")
        df = self.get_m5_historical_data(bars_count=5000)  # 获取5000根K线数据
