...
class FTMOStrategy:
    ...
    def load_data(self):
        ...
        if len(original_columns) == len(new_columns):
            columns_map = dict(zip(original_columns, new_columns))
            self.data.rename(columns=columns_map, inplace=True)
        else:
            # 如果列数不匹配，使用默认命名
            for i in range(min(len(original_columns), len(new_columns))):
                self.data.rename(columns={original_columns[i]: new_columns[i]}, inplace=True)
...
if __name__ == "__main__":
    ...
    strategy = FTMOStrategy(r"EURUSD_H1_历史数据.csv")
