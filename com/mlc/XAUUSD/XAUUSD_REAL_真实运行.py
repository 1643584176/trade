...
# 导入配置加载器
from com.mlc.utils.config_loader import config, get_config_value, get_real_account
...
class FTMORealTimeTrader:
    ...
    def connect_mt5(self):
        ...
        """
        # 使用指定的账户信息连接MT5
        if not mt5.initialize(login=int(ACCOUNT_NUMBER), password=ACCOUNT_PASSWORD, server=ACCOUNT_SERVER):
            error_msg = f"MT5初始化失败，账户: {ACCOUNT_NUMBER}"
        ...
        print(f"[{self.get_current_time()}] 当前连接账户: {ACCOUNT_NUMBER}@{ACCOUNT_SERVER}")
    ...
    def log_current_position_info(self):
        ...
        if hasattr(self, 'current_position_ticket') and self.current_position_ticket:
            positions = mt5.positions_get(ticket=self.current_position_ticket)
            # 过滤出当前币种的持仓
            ...
            if position_info is None or len(position_info) == 0:
                ...
                if deal_history is not None and len(deal_history) > 0:
                    ...
                    for deal in deal_history:
                        if (hasattr(deal, 'position_id') and deal.position_id == self.current_position_ticket and
                            hasattr(deal, 'symbol') and deal.symbol == SYMBOL):  # 添加币种检查
                            # 根据盈亏判断平仓原因
                            if deal.profit < 0:
                                close_reason = "止损"
                            elif deal.profit > 0:
        ...
        try:
            ...
            while True:
                ...
                if not shared_state.can_open_position():
                    self.log_and_print("达到全局风险控制限制，暂停开仓")
                    # 获取全局状态信息用于输出
                    global_stats = shared_state.get_global_stats()
                    self.log_and_print(f"全局状态 - 当日盈亏: {global_stats['daily_pnl']:.2f}, 总盈亏: {global_stats['total_pnl']:.2f}, 净值: {global_stats['equity']:.2f}")
                    if current_position != 0:
                        self.close_position("达到全局风险限制")
                    time.sleep(60)
                    continue
