...
class FTMOMaxConsecutiveLossStrategy:
    ...
    def load_data(self):
        ...
        if not mt5.initialize():
            print("初始化MT5失败, 错误:", mt5.last_error())
            return
        ...
        # 定义时间范围
        timezone = pytz.timezone("Etc/GMT-3")  # 设置时区 (GMT+3)
        ...
        if rates is None:
            print("无法获取历史数据, 错误:", mt5.last_error())
            mt5.shutdown()
            return

        # 转换为DataFrame
        self.data = pd.DataFrame(rates)
        self.data['时间点'] = pd.to_datetime(self.data['time'], unit='s')
    ...
    def calculate_indicators(self):
        ...
        # 计算短期价格动量
        self.data['1周期动量'] = self.data['close'] - self.data['close'].shift(1)
        self.data['3周期动量'] = self.data['close'] - self.data['close'].shift(3)
        self.data['5周期动量'] = self.data['close'] - self.data['close'].shift(5)
        self.data['10周期动量'] = self.data['close'] - self.data['close'].shift(10)

        # 计算K线实体和影线
        self.data['实体'] = abs(self.data['close'] - self.data['open'])
        self.data['上影线'] = self.data['high'] - np.maximum(self.data['open'], self.data['close'])
        self.data['下影线'] = np.minimum(self.data['open'], self.data['close']) - self.data['low']
        self.data['是否阳线'] = self.data['close'] > self.data['open']

        # 计算当日开盘价与相对变化
        daily_open = self.data.groupby('日期')['open'].first()
        self.data['当日开盘价'] = self.data['日期'].map(daily_open)
        self.data['相对开盘价变化'] = self.data['close'] - self.data['当日开盘价']
    ...
    def calculate_weekly_correlations(self):
        ...
        """
        # 计算每周的收盘价
        weekly_close = self.data.groupby('周数')['close'].last().reset_index()
    ...
    def is_reversal_pattern(self, index):
        ...
        # 吞没形态
        if (previous_row['close'] < previous_row['open'] and  # 前一根为阴线
                current_row['close'] > current_row['open'] and  # 当前为阳线
                current_row['open'] < previous_row['close'] and  # 当前开盘价低于前收盘价
                current_row['close'] > previous_row['open']):  # 当前收盘价高于前开盘价
            return 'bullish'

        if (previous_row['close'] > previous_row['open'] and  # 前一根为阳线
                current_row['close'] < current_row['open'] and  # 当前为阴线
                current_row['open'] > previous_row['close'] and  # 当前开盘价高于前收盘价
                current_row['close'] < previous_row['open']):  # 当前收盘价低于前开盘价
            return 'bearish'
        ...
        if index >= 3:
            ...
            # 早晨之星（看涨反转）
            if (previous_3_row['close'] < previous_3_row['open'] and  # 第一根阴线
                    abs(previous_2_row['close'] - previous_2_row['open']) < previous_3_row['实体'] * 0.3 and  # 第二根小实体
                    previous_2_row['low'] < min(previous_3_row['close'], previous_3_row['open']) and
                    current_row['close'] > current_row['open'] and  # 第三根阳线
                    current_row['close'] > previous_2_row['high']):  # 收盘价高于第二根K线
                return 'bullish'

            # 黄昏之星（看跌反转）
            if (previous_3_row['close'] > previous_3_row['open'] and  # 第一根阳线
                    abs(previous_2_row['close'] - previous_2_row['open']) < previous_3_row['实体'] * 0.3 and  # 第二根小实体
                    previous_2_row['high'] > max(previous_3_row['close'], previous_3_row['open']) and
                    current_row['close'] < current_row['open'] and  # 第三根阴线
                    current_row['close'] < previous_2_row['low']):  # 收盘价低于第二根K线
                return 'bearish'
    ...
    def generate_signals(self):
        ...
        # 获取最新K线时间
        latest_time = self.data.iloc[-1]['时间点']
        latest_date = latest_time.date()
        latest_hour = latest_time.hour

        # 初始化交易次数统计
        daily_trades = {latest_date: 0}
        daily_loss = {latest_date: 0.0}
        ...
        for i in range(len(self.data)):
            ...
            if weekday_num == 2:  # 周三
                if reversal == 'bullish':
                    signal = 1
                    strength += 4
                    reasons.append("周三强制看涨反转")
                elif reversal == 'bearish':
                    signal = -1
                    strength += 4
                    reasons.append("周三强制看跌反转")
                # 增加RSI验证条件
                if current_row['RSI'] > 65:  # 超买区域
                    signal = -1
                    strength += 2
                    reasons.append("周三RSI超买验证")
                elif current_row['RSI'] < 35:  # 超卖区域
                    signal = 1
                    strength += 2
                    reasons.append("周三RSI超卖验证")

            # 周五趋势强化
            elif weekday_num == 4:  # Friday (0=Monday, 4=Friday)
                # 增加趋势信号强度
            ...
            # 获取点差信息
            spread = mt5.symbol_info(SYMBOL).spread if mt5.symbol_info(SYMBOL) is not None else 20
            spread_points = spread * 0.1  # 点差单位调整（根据实际点值修改）
            # 添加点差到信号过滤
            if spread_points > 30:
                signal = 0
                reasons.append('点差过大')
            elif spread_points > 20:
                signal = 0 if signal == 0 else signal * -1  # 点差大时反向信号
                reasons.append(f'点差较大({spread_points}点)')
            ...
            if signal != 0:
                if session_type == '亚洲':
                    # 亚洲时段使用更小的止损止盈
                    strength = max(2, strength)  # 强制提高信号强度
                elif session_type == '美洲':
                    # 美洲时段允许更高风险
                    strength = max(3, strength)  # 强制提高信号强度
    ...
    def backtest_strategy(self):
        ...
        trade_pause = 0  # 新增变量，用于控制止盈止损后的暂停交易
        ...
        for i in range(1, len(self.data)):  # 从第二根K线开始
            ...
            if trade_pause > 0:
                trade_pause -= 1
                # 检查是否超过暂停期
                if i - entry_index >= 10:  # 如果超过时间止损周期
                    trade_pause = 0  # 重置暂停标志
                else:
                    continue  # 继续等待
            ...
            # 获取当前时段类型
            session_type = self.get_session_type(current_row['小时'])
            ...
            if ((current_row['Signal'] != 0 and position == 0) or force_trade) and \
                ...
                if daily_losses[date] < daily_max_loss:
                    ...
                    if force_trade and current_row['Signal'] == 0:
                        ...
                        # 只有信号强度足够时才交易（提高门槛）
                        if current_row['Signal_Strength'] >= 4:  # 提高信号强度门槛
                    ...
                    # 只有在确定有仓位时才执行开仓操作
                    if position != 0:
                        entry_price = current_row['close']  # 使用收盘价作为成交价
            ...
            # 如果有仓位，检查是否需要平仓
                ...
                # 时段差异化止损止盈
                if session_type == '亚洲':
                    sl_multiplier = 0.8  # 亚洲时段止损0.8倍ATR
                    tp_multiplier = 1.6  # 止盈1.6倍ATR
                elif session_type == '欧洲':
                    sl_multiplier = 1.0  # 欧洲时段止损1.0倍ATR
                    tp_multiplier = 2.0  # 止盈2.0倍ATR
                else:  # 美洲时段
                    sl_multiplier = 1.2  # 美洲时段止损1.2倍ATR
                    tp_multiplier = 2.4  # 止盈2.4倍ATR

                stop_loss_points = atr * sl_multiplier
                take_profit_points = atr * tp_multiplier
                ...
                if position > 0:  # 多头仓位
                    # 止盈条件
                    if current_row['high'] >= entry_price + take_profit_points:
                        exit_price = entry_price + take_profit_points
                        exit_reason = "止盈"
                    # 止损条件
                    elif current_row['low'] <= entry_price - stop_loss_points:
                    ...
                    # 止盈条件
                    if current_row['low'] <= entry_price - take_profit_points:
                        exit_price = entry_price - take_profit_points
                        exit_reason = "止盈"
                    # 止损条件
                    elif current_row['high'] >= entry_price + stop_loss_points:
                        exit_price = entry_price + stop_loss_points
                        exit_reason = "止损"
                    # 时间止损（缩短至10个周期）
                    elif i - entry_index >= 10:
    ...
    def print_backtest_results(self, trades, equity_curve, daily_trades):
        ...
        if daily_trade_counts:
            ...
            print(f"\n=== 交易频率 ===")
            print(f"平均每日交易数: {avg_daily_trades:.2f}")
            print(f"最低每日交易数: {min(daily_trade_counts)}")
            print(f"最高每日交易数: {max(daily_trade_counts)}")
...
def main():
    # 创建策略实例
    strategy = FTMOMaxConsecutiveLossStrategy()
