...
def get_ftmo_xauusd_orders(start_date=None, end_date=None):
    ...
    :return: XAUUSD订单列表
    """
    # 1. 初始化MT5连接（FTMO需确保终端登录对应账户）
    print("正在连接FTMO MT5终端...")
    ...
    print(f"📅 查询时间范围（UTC）：{start_time_utc} 到 {end_time_utc}")
    ...
    # 9. 输出结果
    if xauusd_orders:
        print(f"✅ 共查询到 {len(xauusd_orders)} 条XAUUSD订单")
    else:
        print(f"ℹ️ 该时间范围无XAUUSD订单")

    if xauusd_deals:
        print(f"✅ 共查询到 {len(xauusd_deals)} 条XAUUSD交易记录（盈亏不为0）")
...
def print_deals_details(deals_list):
    ...
    if deals_list:
        ...
        print("=" * 120)
        print(f"{'交易号':<10} {'订单号':<10} {'品种':<8} {'类型':<6} {'方向':<6} {'价格':<10} {'手数':<6} {'盈亏':<10} {'佣金':<10} {'时间':<20} {'注释'}")
        print("-" * 120)
